...
class ArrayToGoogleSheets
{
  ...
  wid2gid(wid)
  {
    let widVal = wid.length > 3 ? wid.substr(1) : wid;
    let xorval = wid.length > 3 ? 474 : 31578;
    return parseInt(String(widVal), 36) ^ xorval;
  }

  /**
   * @param wid
   * @return {{url: string, gid: number}}
   */
  getUrlObject(wid)
  {
    let gid = this.wid2gid(wid);
    let url = `https://docs.google.com/spreadsheets/d/${this.docKey}/edit#gid=${gid}`;
    return {url, gid};
  }
   ...
   * @param {Array} values
   * @param {Object} options
   * @return {Promise.<{url: {string}, gid: {number}}>}
   */
          ...
          if (!sheet) { // create worksheet if not exist
            this.doc.addWorksheet({
              ...
              this.updateGoogleSheetsData(sheet, values, options).then(() => {
                let urlObject = this.getUrlObject(sheet.id);
                resolve(urlObject);
